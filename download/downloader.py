    ...
    def download(url, proxy=None, data=None, headers=None, encoding=None, retry_times=3, timeout=15):
        ...
        for retry_time in range(0, retry_times):
            try:
                ...
                print("The target web server is not stable,  has retried %d times,url is %s, proxy is %s,"
                            " errorMessage:%s" % (retry_time + 1, url, proxy, e.message))
                continue
            except URLError, e:
                print("Retried %d times, proxy is %s, url is %s" % (retry_time + 1, proxy, url))
                continue
            except Exception, e:
                print("Retried %d times, Download Error %s : %s" % (retry_time + 1, url, e))
                continue
            break
        return html

    @staticmethod
    def downloader_by_requests(url, encode='utf8', proxy_used=None):
        headers = {}
        for key, value in HttpDownloader.default_headers.items():
            if key not in headers:
                if key == "Referer":
        ...
        if proxy_used:
            ...
            proxies['http'] = "http://" + str(proxy_manager.get_a_proxy())
            pass
        for retry_time in range(1, 20):
            try:
                ...
                print("Retried %d times, Download Error %s : %s" % (retry_time, url, e.message))

    @staticmethod
    def download_bs(url, proxy=None, data=None, headers=None, encoding=None, retry_times=3, timeout=20):
        """
        通过HTTP协议和给定参数进行下载
        :param url: 下载的目标URL地址。
        :param proxy: 代理信息。例如：58.60.192.221:8080
